...
namespace ORB_SLAM3_Wrapper
{
    ...
    bool ORBSLAM3Interface::trackMonocularIMU(const sensor_msgs::msg::Image::SharedPtr msgRGB, Sophus::SE3f &Tcw)
    {
    orbAtlas_ = mSLAM_->GetAtlas();
    ...
    bufMutex_.lock();
    // std::cout << "[DEBUG] Checking IMU buffer against image timestamp: " 
        //   << typeConversions_->stampToSec(msgRGB->header.stamp) << std::endl;
    if (!imuBuf_.empty())
    {
        while (!imuBuf_.empty() &&
               typeConversions_->stampToSec(imuBuf_.front()->header.stamp) <= typeConversions_->stampToSec(msgRGB->header.stamp))
        {
            double t = typeConversions_->stampToSec(imuBuf_.front()->header.stamp);
            // std::cout << "[DEBUG] Popping IMU data at time: " << t << std::endl;
            ...
            vImuMeas.push_back(ORB_SLAM3::IMU::Point(acc, gyr, t));
            // std::cout << "[DEBUG] Acc: [" << acc.x << ", " << acc.y << ", " << acc.z << "]"
            //   << " Gyr: [" << gyr.x << ", " << gyr.y << ", " << gyr.z << "]" << std::endl;
            imuBuf_.pop();
        }
    }
    bufMutex_.unlock();
    // std::cout << "[DEBUG] Total IMU measurements collected: " << vImuMeas.size() << std::endl;
    // std::cout<<"HELLO" << endl;

    if (vImuMeas.empty()) {
        // std::cerr << "[ERROR] No IMU data available for the frame at time "
            //   << typeConversions_->stampToSec(msgRGB->header.stamp) << std::endl;
    ...
    // std::cout << "[DEBUG] Attempting to track frame with IMU data" << std::endl;
    Tcw = mSLAM_->TrackMonocular(cvRGB->image, typeConversions_->stampToSec(msgRGB->header.stamp), vImuMeas);

    auto trackingState = mSLAM_->GetTrackingState();
    auto orbLoopClosing = mSLAM_->GetLoopClosing();
    ...
    if (loopClosing_ && orbLoopClosing->mergeDetected())
        ...
        {
            // std::cout << "[DEBUG] TRACKING STATE " << trackingState << std::endl;
    ...
    bool ORBSLAM3Interface::trackMonocular(const sensor_msgs::msg::Image::SharedPtr msgRGB, Sophus::SE3f &Tcw)
    {
        ...
        // std::cout << "[DEBUG] Image callback triggered — frame #" << frame_counter++ 
        //   << " at time " << msgRGB->header.stamp.sec << "." << msgRGB->header.stamp.nanosec << std::endl;
        ...
        // cout << "HERE" << endl;
        // track the frame.
        // std::cout << "[DEBUG] Calling SLAM->TrackMonocular" << std::endl;

        Tcw = mSLAM_->TrackMonocular(cvRGB->image, typeConversions_->stampToSec(msgRGB->header.stamp)); //#TODO: add IMU
        auto currentTrackingState = mSLAM_->GetTrackingState();
        auto orbLoopClosing = mSLAM_->GetLoopClosing();
        if (loopClosing_ && orbLoopClosing->mergeDetected())
        {
    ...
    void ORBSLAM3Interface::saveTrajectoryTUM(const std::string &filename)
    {
        if(mSLAM_)
            mSLAM_->SaveTrajectoryTUM(filename);
    }

    void ORBSLAM3Interface::saveKeyframeTrajectoryTUM(const std::string &filename)
    {
        if(mSLAM_)
            mSLAM_->SaveKeyFrameTrajectoryTUM(filename);
    }

    void ORBSLAM3Interface::saveTrajectoryEuRoc(const std::string &filename)
    {
        if(mSLAM_)
            mSLAM_->SaveTrajectoryEuRoC(filename);
    }

    void ORBSLAM3Interface::saveKeyframeTrajectoryEuRoc(const std::string &filename)
    {
        if(mSLAM_)
            mSLAM_->SaveKeyFrameTrajectoryEuRoC(filename);
    }
    

    void ORBSLAM3Interface::saveMapToPCD(const std::string &filename)
    {
        sensor_msgs::msg::PointCloud2 mapCloud;
        getCurrentMapPoints(mapCloud);

        pcl::PointCloud<pcl::PointXYZ>::Ptr pclCloud(new pcl::PointCloud<pcl::PointXYZ>());
        pcl::fromROSMsg(mapCloud, *pclCloud);

        if (pclCloud->points.empty())
        {
            std::cerr << "No map points to save!" << std::endl;
            return;
        }

        if (pcl::io::savePCDFileBinary(filename, *pclCloud) == -1)
        {
